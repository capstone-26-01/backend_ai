...
flowchart LR
  ...
  Swebench["swebench/runner.py CLI<br/>runtime benchmark"]
  RuntimeJob["Runtime issue harness job<br/>issue, graph, bounded file contents"]
  RuntimePi["llm.pi_issue_runner<br/>runtime Pi harness"]
  Live["OpenCode Zen live path<br/>smoke, sample, or Pi run<br/>opt-in only"]

  subgraph Tools["Runtime bounded Pi tools and fixtures"]
    RuntimeTools["Issue harness tools<br/>get_issue_context<br/>list_repo_files<br/>search_repo_symbols/search_repo_text<br/>read_node_context/read_repo_file"]
  ...
  Matrix --> Swebench
  Swebench --> RuntimeJob
  RuntimeJob --> RuntimePi
  RuntimePi -.-> Live
  RuntimePi --> RuntimeTools
  RuntimeTools --> Fixtures
  RuntimeTools --> Finish
  Finish --> Transcript
  SUT --> Transcript
...
llm/          QA context selection, OpenCode Zen calls, summaries,
              bounded runtime issue harness
harness_eval/ Deterministic issue-map runner evaluation suite and optional live
              Pi/OpenCode benchmark wrappers
...
5. `api.issue_map` ranks seed graph nodes deterministically.
6. When enabled, the bounded Pi issue harness receives only the issue packet,
   stored graph, and bounded file contents. It must read issue context, list
   available files, search repo symbols/text, inspect code or graph neighbors,
   and return investigated node IDs.
7. The backend validates harness node IDs/paths, rebuilds focus graph/code
   context, and falls back to deterministic ranking if the harness is unavailable
...
- `SECRET_KEY`: required outside Django tests.
- `OPENCODE_API_KEY`: required for QA, summaries, and issue explanations.

OpenCode Zen model settings live in `config.yaml`:

- `opencode.model`: default QA/summary/issue-explanation model.
- `opencode.allowed_models`: optional allowlist for request-level QA model
  selection.
- `opencode.max_tokens` and `opencode.timeout_seconds`: provider call limits.
...
- A deterministic evaluator that checks required tool use, forbidden tool use,
  ...
  confidence bounds.
- SWE-bench/model benchmark wrappers that target the active runtime harness:
...
Runtime issue mapping uses the separate bounded harness under `llm/`; the
`harness_eval/` suite remains the judge for model/tool changes.
