...
class SchemaRevisionDocumentationTests(TestCase):
    def test_schema_documents_revision_for_tree_graph_and_qa(self):
        ...
        self.assertIn('프런트엔드에서는 이 엔드포인트 URL을', schema_text)
        self.assertIn('Swagger에서는 원본 GitHub URL을 그대로 입력하세요', schema_text)
        self.assertIn('별도 `share_id`를 먼저 만들 필요가 없습니다', schema_text)
