...
class IssueRelatedNodesResponseSerializer(serializers.Serializer):
    ...
    hypotheses = serializers.JSONField(required=False, help_text='Issue가 어디서 시작됐을 가능성이 높은지에 대한 deterministic 또는 bounded LLM 가설 목록입니다.')
    investigation_path = serializers.JSONField(required=False, help_text='첫 기여자가 순서대로 확인하면 좋은 deterministic 또는 bounded LLM node/file 조사 경로입니다.')
    code_context = serializers.JSONField(required=False, help_text='관련 후보 파일의 bounded code excerpt입니다.')
    confidence = serializers.JSONField(required=False, help_text='Deterministic ranking 또는 bounded LLM explanation confidence 요약입니다.')
