...
class IssueRelatedNodesResponseSerializer(serializers.Serializer):
    ...
    cached = serializers.BooleanField(required=False, help_text='true면 artifact payload에 저장된 issue-map 결과를 반환한 것입니다. false면 이번 요청에서 계산했습니다.')
    cache_key = serializers.CharField(required=False, help_text='artifact payload summaries에 저장되는 issue-map cache key입니다.')
    cache_version = serializers.CharField(required=False, help_text='issue-map 응답 shape/cache semantics 버전입니다.')
